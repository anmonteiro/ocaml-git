...
module Status = struct
  ...
  let pp ppf { result; commands } =
    Fmt.pf ppf "{ @[<hov>result= %a;@ commands= @[<hov>%a@];@] }"
      ...
      result
      Fmt.(Dump.list (pp_commands string)) commands
...
module Decoder = struct
    ...
    let decode_no_ref ~version v decoder =
      ...
      | Some (_, capabilities) ->
          ...
          let k decoder =
            ...
            then ( junk_pkt decoder ; return { Advertised_refs.capabilities; refs = []; version; shallows = [] } decoder )
            else fail decoder (`Invalid_advertised_ref (String.Sub.to_string pkt)) in
          prompt_pkt k decoder
  ...
  let decode_flush decoder =
    let k decoder =
      ...
      else fail decoder (`Unexpected_pkt_line (String.Sub.to_string pkt)) in
    prompt_pkt k decoder
    ...
    let command decoder =
      ...
      if String.Sub.length pkt = 0 then Stdlib.Ok None
        ...
        | res :: reference :: rest -> (
            ...
            | "ok" ->
              Stdlib.Ok (Some (`OK (String.Sub.to_string reference)))
            | "ng" ->
            ...
            | "ff" ->
              Stdlib.Ok (Some (`FF (String.Sub.to_string reference)))
            | _ ->
